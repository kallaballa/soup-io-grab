...
LUA_SHA1 = get_hash(os.path.join(CWD, 'soup-io.lua'))
...
class WgetArgs(object):
    def realize(self, item):
        wget_args = [
            ...
            '--warc-header', 'soup-io-dld-script-version: ' + VERSION,
            '--warc-header', ItemInterpolation('soup-io-item: %(item_name)s'),
...
pipeline = Pipeline(
    ...
    GetItemFromTracker('http://%s/%s' % (TRACKER_HOST, TRACKER_ID), downloader,
        VERSION),
    PrepareDirectories(warc_prefix='soup-io'),
    WgetDownload(
        WgetArgs(),
        max_tries=2,
